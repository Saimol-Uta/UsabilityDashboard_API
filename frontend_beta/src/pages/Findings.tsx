...
export default function Findings() {
  ...
  const emptyForm = {
    testPlanId: 1, description: '', frequency: '', severity: 'Media',
    priority: 'Media', category: '', recommendation: '', tool: 'WAVE'
  }
  const [form, setForm] = useState(emptyForm)

  const fetchFindings = () => {
    setLoading(true)
    findingsApi.getAll(1).then(res => setFindings(res.data)).finally(() => setLoading(false))
  }
  ...
  const handleEdit = (f: any) => {
    setForm({
      ...
      severity: f.severity, priority: f.priority, category: f.category,
      recommendation: f.recommendation, tool: f.tool
    })
    setEditId(f.id)
    setShowForm(true)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        await findingsApi.update(editId, { id: editId, ...form })
        addToast('Hallazgo actualizado', 'success')
      } else {
        await findingsApi.create(form)
        addToast('Hallazgo creado', 'success')
      }
      resetForm(); fetchFindings()
    } catch { addToast('Error al guardar', 'error') }
  }

  const handleDelete = async (id: number) => {
    if (!confirm('¿Eliminar este hallazgo?')) return
    try { await findingsApi.delete(id); addToast('Hallazgo eliminado', 'success'); fetchFindings() }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {filtered.map((finding: any) => (
            <div key={finding.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-rise hover:shadow-md transition-shadow">
              <div className={`h-1.5 w-full ${finding.severity === 'Alta' ? 'bg-gradient-to-r from-red-500 to-red-400' : finding.severity === 'Media' ? 'bg-gradient-to-r from-blue-500 to-blue-400' : 'bg-gradient-to-r from-emerald-500 to-emerald-400'}`} />
              <div className="p-4">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className={`badge ${finding.severity === 'Alta' ? 'badge-alta' : finding.severity === 'Media' ? 'badge-media' : 'badge-baja'}`}>
                    {finding.severity === 'Alta' ? <AlertCircle size={10} /> : <AlertTriangle size={10} />}
                    {finding.severity}
                  </span>
                </div>
                ...
                <div className="mt-3 pt-2 border-t border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-2 text-[10px] text-slate-400">
                    ...
                    <span>·</span>
                    <span>Prioridad: {finding.priority}</span>
