...
export default function Tasks() {
  ...
  const [form, setForm] = useState({
    testPlanId: 1, taskNumber: 0, scenario: '', expectedResult: '', successCriteria: '', maxTimeSeconds: 120
  })
  const { addToast } = useToast()

  const fetchTasks = () => {
    setLoading(true)
    testTasksApi.getAll(1).then(res => setTasks(res.data)).finally(() => setLoading(false))
  }
  useEffect(() => { fetchTasks() }, [])

  const resetForm = () => {
    setForm({ testPlanId: 1, taskNumber: tasks.length + 1, scenario: '', expectedResult: '', successCriteria: '', maxTimeSeconds: 120 })
    setEditId(null)
    setShowForm(false)
  }
  ...
  const handleEdit = (task: any) => {
    setForm({
      ...
      expectedResult: task.expectedResult, successCriteria: task.successCriteria, maxTimeSeconds: task.maxTimeSeconds
    })
    setEditId(task.id)
    setShowForm(true)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        await testTasksApi.update(editId, { id: editId, ...form })
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <button onClick={() => { setForm(f => ({ ...f, taskNumber: tasks.length + 1 })); setEditId(null); setShowForm(true) }} className="btn btn-primary">
