                            ...
                            {phases.map((phase, idx) => {
                                ...
                                return (
                                    <div key={idx} className="space-y-1.5">
                                        {/* Phase header */}
                                        <div className={`flex items-center gap-2 px-2 py-1.5 rounded-lg transition-colors ${phaseComplete ? 'bg-emerald-50 border border-emerald-200' :
                                                phaseLocked ? 'bg-slate-50 border border-slate-200' :
                                                    'border border-transparent'
                                            }`}>
                                            {/* ACCESIBILIDAD: SVG de estado de fase con title descriptivo (WCAG 1.1.1) */}
                                            <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 ${phaseComplete ? 'bg-emerald-500' : phaseLocked ? 'bg-slate-300' : 'bg-blue-400'
                                                }`} aria-hidden="true">
                                                {phaseComplete ? (
                                            ...
                                            <span className={`text-[10px] uppercase tracking-[0.12em] font-bold flex-1 ${phaseComplete ? 'text-emerald-700' : phaseLocked ? 'text-slate-400' : 'text-blue-600'
                                                }`}>
                                                {phase.title}
                                            ...
                                            {phase.items.map(item => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                            <div className={`text-[13px] font-medium transition-colors ${sectionDone[item.sectionKey] ? 'text-emerald-800 group-hover:text-emerald-900' : 'text-slate-600 group-hover:text-slate-800'
                                                                }`}>{item.label}</div>
                                                            {/* ACCESIBILIDAD: contraste mejorado de text-slate-400 a text-slate-500 (ratio 4.6:1, WCAG 1.4.3) */}
                                                            <div className={`text-[11px] mt-0.5 truncate ${sectionDone[item.sectionKey] ? 'text-emerald-600' : 'text-slate-500'
                                                                }`}>{item.detail}</div>
                                                        </div>
                                                    </NavLink>
                                                )
